(() => {
  function initRealCbct() {
    const anchor = document.getElementById('realRadiographCase');
    if (!anchor) return false;
    if (document.getElementById('realCbctCase')) return true;
  ...
  if (initRealCbct()) return;
  const root = document.querySelector('.simMain') || document.body;
  const observer = new MutationObserver(() => {
    if (initRealCbct()) observer.disconnect();
  });
