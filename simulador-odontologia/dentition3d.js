...
function createTooth(part,fdi){
  const [lo,hi]=part.bounds;const w=Math.max(.002,hi[0]-lo[0]),h=Math.max(.004,hi[1]-lo[1]),d=Math.max(.002,hi[2]-lo[2]);const upper=fdi<30;const group=new THREE.Group();group.position.set((lo[0]+hi[0])/2,(lo[1]+hi[1])/2,(lo[2]+hi[2])/2);group.userData={fdi,partName:part.name,synthetic:!!part.synthetic};
...
function extrapolateThird(items,q){
  const second=items.find(x=>x.fdi===q*10+7);const first=items.find(x=>x.fdi===q*10+6);if(!second)return null;
  const [lo2,hi2]=second.part.bounds;let dx=0,dz=.009;
  if(first){const c1=[(first.part.bounds[0][0]+first.part.bounds[1][0])/2,(first.part.bounds[0][2]+first.part.bounds[1][2])/2],c2=[(lo2[0]+hi2[0])/2,(lo2[2]+hi2[2])/2];dx=(c2[0]-c1[0])*.92;dz=(c2[1]-c1[1])*.92;}
  const bounds=[[lo2[0]+dx,lo2[1],lo2[2]+dz],[hi2[0]+dx,hi2[1],hi2[2]+dz]];
  return {part:{name:`Third molar FDI ${q*10+8} · educational proxy`,bounds,synthetic:true},fdi:q*10+8};
}

fetch(MANIFEST).then(r=>{if(!r.ok)throw new Error(`Manifest ${r.status}`);return r.json();}).then(data=>{
  const items=(data.parts||[]).map(part=>({part,fdi:fdiFromName(part.name||'')})).filter(x=>x.fdi&&Array.isArray(x.part.bounds));if(!items.length)throw new Error('No se encontraron dientes en el atlas');
  const present=new Set(items.map(x=>x.fdi));for(const q of [1,2,3,4])if(!present.has(q*10+8)){const proxy=extrapolateThird(items,q);if(proxy)items.push(proxy);}
  items.sort((a,b)=>a.fdi-b.fdi);const seen=new Set();for(const {part,fdi} of items){if(seen.has(fdi))continue;seen.add(fdi);teeth.push(createTooth(part,fdi));}
  list.innerHTML=teeth.map(g=>`<button type="button" data-fdi="${g.userData.fdi}">${g.userData.fdi}${g.userData.synthetic?'*':''}</button>`).join('');list.querySelectorAll('[data-fdi]').forEach(b=>b.addEventListener('click',()=>select(teeth.find(g=>g.userData.fdi===Number(b.dataset.fdi)))));fitAll();requestRender();
