...
function helpCard(x){return '<button class="helpCard" data-sec="'+x.section+'"><span class="icon">'+x.icon+'</span><h3>'+esc(x.label)+'</h3><p>'+esc(x.description)+'</p></button>'}function bindHelpCards(root=document){root.querySelectorAll('[data-sec]').forEach(b=>b.onclick=()=>helpCenter(b.dataset.sec))}function renderHelp(q=''){const s=q.toLowerCase().trim(),xs=S.help.filter(x=>!s||[x.label,x.description,x.section,x.group_label].some(v=>String(v||'').toLowerCase().includes(s)));const groups=[...new Map(xs.map(x=>[x.group_key,{key:x.group_key,label:x.group_label,description:x.group_description,order:x.group_sort_order}])).values()].sort((a,b)=>a.order-b.order);$('#helpSections').innerHTML=groups.map(g=>{const items=xs.filter(x=>x.group_key===g.key);return '<section class="helpGroup"><div class="helpGroupHead"><div><h2>'+esc(g.label)+'</h2><p>'+esc(g.description)+'</p></div></div><div class="grid3">'+items.map(helpCard).join('')+'</div></section>'}).join('');bindHelpCards($('#helpSections'))}function renderFeatured(){const preferred=['boleto','becas','siu','examenes','reglamento','calendario','ingreso','campus'];const seen=new Set(),xs=[];for(const key of preferred){const x=S.help.find(i=>i.section===key);if(x&&!seen.has(x.section)){seen.add(x.section);xs.push(x)}}for(const x of S.help.filter(i=>i.featured)){if(!seen.has(x.section)){seen.add(x.section);xs.push(x)}}$('#featuredHelp').innerHTML=xs.slice(0,8).map(helpCard).join('');bindHelpCards($('#featuredHelp'))}function fmtDate(v){if(!v)return'';return new Date(String(v).slice(0,10)+'T12:00:00').toLocaleDateString('es-AR',{day:'2-digit',month:'short',year:'numeric'})}function answerHtml(a){const art=a.article_number?'<div class="answer"><b>Artículo '+esc(a.article_number)+'</b><br>'+esc(a.article_excerpt||'')+'</div>':'';let badges='';if(a.time_sensitive)badges+=' <span class="badge amber">Información temporal</span>';if(a.valid_through)badges+=' <span class="badge">Vigente hasta '+fmtDate(a.valid_through)+'</span>';return '<div class="answer"><div class="meta">'+badges+'</div><h3>'+esc(a.title||a.question)+'</h3><p>'+esc(a.body||a.answer)+'</p>'+art+(a.source_url?'<a href="'+esc(a.source_url)+'" target="_blank" rel="noopener noreferrer">Ver fuente oficial →</a>':'')+'</div>'}async function helpCenter(sec){track('Help Topic Open',{section:sec});const meta=S.help.find(x=>x.section===sec);open('<span class="ey">Centro de ayuda</span><h2>'+esc(meta?.label||sec)+'</h2><div id="ql"></div>');try{const rows=await rpc('get_help_center',{section_key:sec});const renderQuestions=()=>{const ql=$('#ql');if(!ql)return;ql.innerHTML=rows.map((x,i)=>'<button class="qbtn" data-i="'+i+'">'+esc(x.question)+(x.article_number?' · Art. '+x.article_number:'')+'</button>').join('');ql.querySelectorAll('.qbtn').forEach(b=>b.onclick=()=>renderAnswer(+b.dataset.i))};const renderAnswer=i=>{const ql=$('#ql'),row=rows[i];if(!ql||!row)return;ql.innerHTML='<button type="button" class="btn secondary helpBack">← Volver a las preguntas</button>'+answerHtml({...row,title:row.question,body:row.answer});ql.querySelector('.helpBack').onclick=renderQuestions};renderQuestions()}catch{}}async function loadSug(el){try{const x=await rpc('get_assistant_suggestions',{section_key:null,result_limit:8});el.innerHTML=x.map(i=>'<button class="chip">'+esc(i.question)+'</button>').join('');el.querySelectorAll('button').forEach(b=>b.onclick=()=>assistant(b.textContent))}catch{}}async function assistant(seed=''){track('Assistant Open',{seeded:Boolean(seed)});open('<span class="ey">Respuestas con fuente</span><h2>Preguntale a Ayuda para el Alumno</h2><div class="search"><input id="aq" value="'+esc(seed)+'" placeholder="Escribí tu pregunta"><button id="ab" class="btn primary">Preguntar</button></div><div id="asug" class="chips"></div><div id="ares"></div>');$('#ab').onclick=()=>ask($('#aq').value);$('#aq').onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();ask($('#aq').value)}};loadSug($('#asug'));if(seed)ask(seed)}async function ask(q){if(!q.trim())return;track('Assistant Search');$('#ares').innerHTML='<div class="answer">Buscando…</div>';try{const d=await rpc('get_assistant_response',{search_text:q,related_limit:3});if(!d.found){$('#ares').innerHTML='<div class="answer"><h3>No encontramos información para esa consulta</h3><p>Probá con otras palabras o entrá a una categoría.</p></div>';return}$('#ares').innerHTML=answerHtml(d.answer)+(d.related?.length?'<div class="related"><b>También puede servirte</b>'+d.related.map(x=>'<button data-r="'+esc(x.title)+'">'+esc(x.title)+'</button>').join('')+'</div>':'');$$('[data-r]').forEach(b=>b.onclick=()=>ask(b.dataset.r))}catch{}}$$('[data-open="assistant"]').forEach(b=>b.onclick=()=>assistant());const UPLOAD_MAX=25*1024*1024,UPLOAD_ACCEPT='.pdf,.jpg,.jpeg,.png,.webp,.doc,.docx,.ppt,.pptx,.xls,.xlsx';
...
$('#groupSearch').oninput=e=>groups(e.target.value);function events(){const fmt=d=>d?new Date(d+'T12:00:00').toLocaleDateString('es-AR',{day:'2-digit',month:'short',year:'numeric'}):'';const today=(()=>{const p=new Intl.DateTimeFormat('en-CA',{timeZone:'America/Argentina/Buenos_Aires',year:'numeric',month:'2-digit',day:'2-digit'}).formatToParts(new Date()),o={};p.forEach(x=>o[x.type]=x.value);return o.year+'-'+o.month+'-'+o.day})();const score=x=>{const t=(String(x.title||'')+' '+String(x.description||'')).toLowerCase();let s=0;if(/beca|progresar/.test(t))s+=70;if(/inscripci[oó]n/.test(t))s+=60;if(/examen|final|mesa/.test(t))s+=55;if(/estudiante|clases|cursada/.test(t))s+=40;if(/proyecto|i\+d|extensi[oó]n/.test(t))s-=20;return s};$('#events').innerHTML=S.e.map(x=>'<article class="event"><div class="date">'+fmt(x.start_date)+'</div><div><b>'+esc(x.title)+'</b><p>'+esc(x.description||'')+'</p></div></article>').join('');const upcoming=S.e.filter(x=>(x.end_date||x.start_date)>=today).sort((a,b)=>score(b)-score(a)||String(a.start_date).localeCompare(String(b.start_date)));$('#homeEvents').innerHTML=upcoming.slice(0,3).map(x=>'<article class="event"><div class="date">'+fmt(x.start_date)+'</div><div><b>'+esc(x.title)+'</b></div></article>').join('')}
